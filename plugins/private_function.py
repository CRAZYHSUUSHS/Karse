    ...
    if m.text == "المطورين":
        if sudo2(m):
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [crezu](tg://user?id={super_sudoers[0]})\n" +
...
#𝙰𝙱𝙳𝙾𝚘 : تم التعديل بواسطة 🫧⋅
    ...
    if m.text == (get_db_botname() or "كريزي"):
        texting = ["اؤمر " + (get_db_botname() or "كريزي") + " شتريد؟❤️🥺",
                   "اى يقلب " + (get_db_botname() or "كريزي") + "❤️",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "كريزي1") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
...
◍ رابط حذف [Snspchat](https://accounts.snapchat.com/accounts/login?continue=https%3A%2F%2Faccounts.snapchat.com%2Faccounts%2Fdeleteaccount) √
    ...
    if re.match("^معني (.*)$", str(m.text)) or re.match("^معنى (.*)$", str(m.text)):
        r = requests.get("https://leadercrezu.herokuapp.com/name.php?leocrezu=" + m.text[5:])
        rj = r.json()
